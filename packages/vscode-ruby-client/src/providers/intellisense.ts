...
export function registerIntellisenseProvider(ctx: ExtensionContext) {
	...
	if (vscode.workspace.getConfiguration('ruby').intellisense == 'rubyLocate') {
		...
		const lib_paths = vscode.workspace.getConfiguration("ruby.locate").lib_paths
		const workspacePaths = vscode.workspace.workspaceFolders.map(p => p.uri.fsPath)
		const target_paths = [...new Set(workspacePaths.concat(lib_paths))]
		...
		target_paths.forEach((path) => {
			...
			let locate = new Locate(path, settings);
			refreshLocate();
			ctx.subscriptions.push(vscode.commands.registerCommand('ruby.reloadProject:' + path, refreshLocate));
			...
			const defProvider = {
				provideDefinition: (doc, pos) => {
					...
					return locate.find(txt).then((matches) => {
						if (
							doc.fileName.match(path + '/') || lib_paths.find((p) => { return matches[0] && matches[0].file.match(p + '/')})
						) {
							return matches.map(locationConverter);
						}
					});
				}
			};
